...
if (getPassed('fromURL') && postPassed('url')) {
    $path = $_POST['url'];
    $ext = substr(strrchr($path, '.'), 1);
    $filename = uniqid().'.'.$ext;
    $ch = curl_init($path);
    $f = fopen('resources/'.$filename, 'wb');
    ...
    if (!$result) {
	exit(json_encode(array('error' => true, 'desc' => $uploader->getErrorMsg())));
    }

    echo json_encode(array('error' => false, 'id' => $uploader->getFileName()));
}
